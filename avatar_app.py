...
if "avatar_text" not in st.session_state:
    st.session_state["avatar_text"] = "안녕하세요? 만나서 반갑습니다. 주식회사 솔라시우에서 현재 아바타 생성 시험 중입니다."


if st.sidebar.button("대본 생성하기"):
    client = OpenAI(api_key=OPENAI_API_KEY)
    st.session_state["avatar_text"] = client.responses.create(model="gpt-4o", input="70대 아버지가 멀리있는 아들, 딸들에게 보내는 안부인사 100자 이내로 작성해줘.").output_text
    
speech_avatar = st.sidebar.text_area(label="🔠아바타 대사를 입력하세요!", value=st.session_state["avatar_text"] , height=100, max_chars=100)
